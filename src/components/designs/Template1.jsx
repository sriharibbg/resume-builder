...
const Template1 = () => {
 ...
 const  getTemplateDetailEditByUser = async (uid, resumeId) => {
  if (!uid || !resumeId) throw new Error("Missing uid or resumeId");

  const response = await apis.get("/get-templateDetailEditByUser", {
    params: { uid, resumeId },
  });
 ...
 const {
  ...
  try {
    ...
    const response = await apis.post("/resumes/save", payload);
