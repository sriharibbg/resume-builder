...
<header className='w-full flex items-center justify-between px-4 py-3 lg:px-8 border-b border-gray-300 bg-bgPrimary z-50 gap-12 sticky top-0'>
    ...
    (
        <React.Fragment>
        {data? <motion.div {...FadeInOutWithOpacity} className='relative' onClick={()=>setIsMenu(!isMenu)}> 
            {data?.photoURL?
...
{isMenu &&  <motion.div 
 key="menu"
        {...slideUpMenu}
className="absolute px-4 py-3 rounded-md bg-white right-0 top-14 flex flex-col items-center justify-start gap-3 w-64 pt-12" onMouseLeave={() => setIsMenu(false)} >
    {data?.photoURL ? (
...
<div className='w-full px-2 py-2 border-t border-gray-300 flex items-center justify-between group cursor-pointer' onClick={signOutUser}>
    ...
    <HiLogout className='group-hover:text-txtDark text-txtLight'/>
        ...
        :<Link to={'/auth'}>
        <motion.button className='px-4 py-2 rounded-md border border-gray-300 bg-gray-200 hover:shadow-md active:scale-95 duration-150' type='button' {...FadeInOutWithOpacity}>Login</motion.button>
        </Link>}
        </React.Fragment>
    )
}
